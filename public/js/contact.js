...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function closeMobile() {
    ...
    if (mobileBackdrop) mobileBackdrop.style.opacity = '';
  ...
  /* ---------- Accessible Accordion (generic for any #faq) ---------- */
  const setupAccordion = (rootSel) => {
    const root = document.querySelector(rootSel);
    if (!root) return;
    const items = $$('.q', root);
    items.forEach((item, idx) => {
      const btn = $('.q-btn', item);
      const panel = $('.answer', item);
      if (!btn || !panel) return;
      btn.setAttribute('aria-controls', `${rootSel.replace('#','')}-panel-${idx}`);
      panel.id = `${rootSel.replace('#','')}-panel-${idx}`;
      ...
      btn.addEventListener('click', () => {
        ...
        items.forEach(i => {
          const b = $('.q-btn', i);
          const p = $('.answer', i);
          if (b !== btn) { b.setAttribute('aria-expanded','false'); i.classList.remove('open'); p.hidden = true; }
        });
        if (expanded) { btn.setAttribute('aria-expanded','false'); item.classList.remove('open'); panel.hidden = true; }
        else { btn.setAttribute('aria-expanded','true'); item.classList.add('open'); panel.hidden = false;
               setTimeout(()=> item.scrollIntoView({behavior: 'smooth', block: 'center'}), 80); }
      });

      // keyboard nav
      btn.addEventListener('keydown', e => {
        if (e.key === 'ArrowDown') { e.preventDefault(); items[idx+1]?.querySelector('.q-btn')?.focus(); }
        if (e.key === 'ArrowUp') { e.preventDefault(); items[idx-1]?.querySelector('.q-btn')?.focus(); }
      });
    });
  };
  setupAccordion('#faq');
  setupAccordion('#faq-contact');

  /* ---------- Counters (animate when visible) ---------- */
  const counters = Array.from(document.querySelectorAll('.stat .num'));
  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const animateCount = (el, to) => {
    if (reduceMotion) { el.textContent = to >= 1000 ? '+' + (to/1000).toFixed(1)+'k' : to.toString(); return; }
    const start = 0, duration = 1100;
    let startTime = null;
    function step(ts) {
      if (!startTime) startTime = ts;
      const progress = Math.min((ts - startTime)/duration, 1);
      const value = Math.floor(progress*(to-start) + start);
      el.textContent = to >= 1000 ? (value/1000).toFixed(1) + 'k' : value.toString();
      if (progress < 1) requestAnimationFrame(step);
      else el.textContent = to >= 1000 ? '+' + (to/1000).toFixed(1) + 'k' : to.toString();
    }
    ...
    const obs = new IntersectionObserver((entries, o) => {
      entries.forEach(en => {
        if (en.isIntersecting) {
          const el = en.target;
          const target = Number(el.dataset.target) || parseInt(el.textContent.replace(/\D/g,''),10) || 0;
  ...
  } else counters.forEach(c => animateCount(c, Number(c.dataset.target) || 0));

  /* ---------- Image fallback + LQIP idea ---------- */
  const heroImg = document.querySelector('.hero-img');
  if (heroImg) {
    heroImg.addEventListener('error', () => {
      heroImg.src = 'data:image/svg+xml;utf8,' + encodeURIComponent(
        `<svg xmlns="http://www.w3.org/2000/svg" width="800" height="450" viewBox="0 0 800 450"><rect width="800" height="450" fill="#0b1220"/><text x="50%" y="50%" fill="#9fb7ff" font-family="Arial" font-size="20" text-anchor="middle" dominant-baseline="middle">تصویر در دسترس نیست</text></svg>`
      );
      heroImg.classList.add('img-fallback');
    });
  }
  ...
  document.querySelectorAll('a[href^="#"]').forEach(a => {
    a.addEventListener('click', e => {
      ...
      if (el) { e.preventDefault(); el.scrollIntoView({behavior:'smooth', block:'center'}); closeMobile(); }
    });
  });
  ...
  if (form && status) {
    const emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    const setError = (id, txt) => { const e = document.getElementById(id); if (e) e.textContent = txt; };
    const clearAll = () => { setError('err_name',''); setError('err_email',''); setError('err_message',''); status.hidden = true; };
    ...
    form.addEventListener('submit', async (ev) => {
      ...
      // honeypot
      if (form.hp && form.hp.value) { status.hidden = false; status.textContent = 'درخواست نامعتبر'; status.style.color = '#ff7b7b'; return; }

      const name = form.name.value.trim();
      const email = form.email.value.trim();
      const subject = form.subject?.value.trim() || '';
      const message = form.message.value.trim();
      const type = form.type?.value || 'general';

      let hasError = false;
      if (!name || name.length < 2) { setError('err_name','نام معتبر وارد کنید.'); hasError = true; }
      if (!email || !emailRe.test(email)) { setError('err_email','ایمیل معتبر وارد کنید.'); hasError = true; }
      if (!message || message.length < 10) { setError('err_message','پیام باید حداقل ۱۰ حرف باشد.'); hasError = true; }
      if (hasError) { status.hidden = false; status.textContent = 'لطفاً خطاهای فرم را اصلاح کنید.'; status.style.color = '#ffd166'; return; }
      ...
      const payload = { name, email, subject, message, type, meta: { url: location.href, ts: Date.now() } };
      const endpoint = form.dataset.apiEndpoint || '/api/contact';

      try {
        // real request (uncomment when API available)
        // const res = await fetch(endpoint, { method:'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
        // const result = await res.json();

        // mock delay for UX
        await new Promise(r => setTimeout(r, 900));
        const result = { ok: true, ticket: 'CNT-12345' };

        if (result && (result.ok === true || result.status === 'ok')) {
          status.hidden = false;
          status.textContent = 'پیام شما ارسال شد. شماره پیگیری: ' + (result.ticket || '');
        ...
        status.hidden = false;
        status.textContent = 'خطا در ارسال پیام — لطفاً دوباره تلاش کنید یا مستقیماً ایمیل بزنید.';
  ...
  /* ---------- FAQ-contact toggle (compact) ---------- */
  $$('#faq-contact .q .q-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const q = btn.closest('.q'); const ans = q.querySelector('.answer');
      const open = ans.hidden === false;
      $$('#faq-contact .answer').forEach(a => a.hidden = true);
      $$('#faq-contact .q').forEach(x => x.classList.remove('open'));
      if (!open) { ans.hidden = false; q.classList.add('open'); q.scrollIntoView({behavior:'smooth', block:'center'}); }
    });
  });

  /* ---------- small UI niceties ---------- */
  // Make wallet menu toggle
  const walletButton = $('#walletButton');
  const walletMenu = $('#walletMenu');
  walletButton?.addEventListener('click', (e) => {
    e.stopPropagation();
    if (!walletMenu) return;
    const shown = walletMenu.classList.toggle('hidden');
    walletMenu.style.display = walletMenu.classList.contains('hidden') ? 'none' : 'block';
  });
  // close wallet menu on outside click
  document.addEventListener('click', (e) => {
    if (walletMenu && !walletMenu.classList.contains('hidden')) {
      const inside = walletMenu.contains(e.target) || (walletButton && walletButton.contains(e.target));
      if (!inside) { walletMenu.classList.add('hidden'); walletMenu.style.display = 'none'; }
    }
  });
});
