// about.js - mobile menu, accessible accordion, counters, and small helpers
document.addEventListener("DOMContentLoaded", () => {
  ...
  faqItems.forEach((item, index) => {
    ...
    btn.addEventListener("keydown", (e) => {
      if (e.key === "ArrowDown") {
        e.preventDefault();
        faqItems[index + 1]?.querySelector(".q-btn")?.focus();
      }
      if (e.key === "ArrowUp") {
        e.preventDefault();
        faqItems[index - 1]?.querySelector(".q-btn")?.focus();
  ...
  function animateCount(el, to) {
    ...
    function step(ts) {
      ...
      el.textContent =
        to >= 1000 ? (value / 1000).toFixed(1) + "k" : value.toString();
      if (progress < 1) requestAnimationFrame(step);
      else {
        // final formatting for big numbers
        el.textContent =
          to >= 1000 ? "+" + (to / 1000).toFixed(1) + "k" : to.toString();
      }
    }
  ...
  if ("IntersectionObserver" in window && counters.length) {
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            const target =
              ...
              parseInt(el.textContent.replace(/\D/g, "") || 0, 10);
    ...
    counters.forEach((c) =>
      animateCount(c, Number(c.getAttribute("data-target")) || 0)
    );
  }

  /* ---------- Smooth internal anchors (if any) ---------- */
...
// main-enhance.js — اضافه کن به انتهای js/about.js یا در همان فایل
document.addEventListener('DOMContentLoaded', () => {
  // fallback image: اگر تصویر موجود نبود، یک placeholder SVG می‌گذارد
  ...
  if (heroImg) {
    heroImg.addEventListener('error', () => {
      heroImg.src = 'data:image/svg+xml;utf8,' + encodeURIComponent(
        `<svg xmlns="http://www.w3.org/2000/svg" width="800" height="450" viewBox="0 0 800 450">
           ...
           <g fill="#7c4dff" font-family="Arial, Helvetica, sans-serif" font-size="20">
             <text x="50%" y="48%" dominant-baseline="middle" text-anchor="middle" fill="#9fb7ff">تصویر در دسترس نیست</text>
           </g>
         </svg>`
      );
    ...
    // simple LQIP idea: if browser supports IntersectionObserver we keep lazy behavior
    if ('IntersectionObserver' in window) {
      ...
      // if we used data-src pattern (not necessary because we used src). keep for robustness:
      if (heroImg.dataset.src) io.observe(heroImg);
    }
  }

  // ensure main is horizontally centered — small runtime sanity check (no layout change)
  const mainInner = document.querySelector('.main-inner');
  if (mainInner) {
    mainInner.style.maxWidth = mainInner.style.maxWidth || '980px';
    mainInner.style.marginLeft = 'auto';
    mainInner.style.marginRight = 'auto';
  }
});
