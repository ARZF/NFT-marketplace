...
function viewOwnedNFTs() {
    ...
    const owned = lastListings.filter(l => Number(l.chain_id) === Number(currentChainId) && ((l.seller_address && l.seller_address.toLowerCase() === addr) || (l.owner && l.owner.toLowerCase() === addr)));
...
function renderOwnedNFTsModal(listings) {
    ...
    if (!listings.length) {
        ...
        listings.forEach((listing) => {
            ...
            template.querySelector('[data-field="price"]').textContent = `${listing.price_eth} ${CURRENCY}`;
